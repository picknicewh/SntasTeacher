...
public class StatusBarUtil {
    public static final int DEFAULT_STATUS_BAR_ALPHA = 112;//透明
    private static final int DEFAULT_STATUS_BAR_NO_ALPHA = 0;//不透明
    ...
    public static void setColor(Activity activity, @ColorInt int color) {
        setColor(activity, color, DEFAULT_STATUS_BAR_NO_ALPHA);
    }
     ...
     */
    @Deprecated
    public static void ForDrawerLayoutDiff(Activity activity, DrawerLayout drawerLayout, @ColorInt int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            // 生成一个状态栏大小的矩形
            ViewGroup contentLayout = (ViewGroup) drawerLayout.getChildAt(0);
